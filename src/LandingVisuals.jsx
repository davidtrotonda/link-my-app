...
export function PricingPageExtras() {
  ...
  const compareRows = t("pricingExtras.compareRows", { returnObjects: true });
  const compareFallback = [
    { feature: "Smart links activos", free: "1", pro: "Ilimitados" },
    { feature: "Códigos QR", free: "1 automático", pro: "Ilimitados" },
    { feature: "Estadísticas de clics", free: "Básicas", pro: "Completas por dispositivo, fuente y campaña" },
    { feature: "Slugs personalizados", free: false, pro: true },
    { feature: "Editar destinos sin reimprimir QR", free: false, pro: true },
    { feature: "Smart link por canal (Instagram, TikTok, QR...)", free: false, pro: true },
    { feature: "Historial completo de clics", free: false, pro: true },
    { feature: "Panel multi-app", free: false, pro: true },
    { feature: "Soporte por email", free: true, pro: true },
  ];
  const rows = Array.isArray(compareRows) && compareRows.length > 0 ? compareRows : compareFallback;
  ...
  function renderCell(value) {
    if (value === true) {
      return (
        <span className="inline-grid h-6 w-6 place-items-center rounded-full bg-black text-white">
          <Check size={13} strokeWidth={3} />
        </span>
      );
    }
    if (value === false) {
      return <span className="text-base font-black text-black/25">—</span>;
    }
    return <span className="text-sm font-bold text-black/80">{value}</span>;
  }

  return (
    <>
      {/* ─── Detailed comparison table ─── */}
      <section className="relative w-full bg-[#fafaf8] px-5 py-20 text-black md:px-8 md:py-24">
        <div className="mx-auto max-w-[1120px]">
          <div className="mx-auto max-w-2xl text-center">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-black/45">
              {t("pricingExtras.compareTag", "Comparativa completa")}
            </p>
            <h2 className="mt-3 text-[clamp(28px,4.4vw,48px)] font-black leading-[0.95] tracking-[-0.05em]">
              {t("pricingExtras.compareTitle", "Qué incluye exactamente cada plan")}
            </h2>
          </div>
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-3">
            {reasonItems.map((r) => (
              ...
              >
                ...
                <h3 className="mt-5 text-[18px] font-black tracking-[-0.025em]">{r.title}</h3>
                <p className="mt-3 text-sm font-medium leading-7 text-black/60">{r.text}</p>
              </article>
            ))}
          </div>
        </div>
                ...
                >
                  <div className="flex items-center justify-between gap-5">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className="text-base font-black tracking-[-0.02em] text-black md:text-lg">{f.q}</h3>
                    </div>
                    <ChevronDown
                      size={20}
                  ...
                  <div className={`grid transition-all duration-300 ${isOpen ? "grid-rows-[1fr] pt-4" : "grid-rows-[0fr]"}`}>
                    <div className="overflow-hidden">
                      <p className="pl-12 text-sm font-medium leading-7 text-black/55 md:text-base">{f.a}</p>
                    </div>
                  </div>
                </button>
              );
      ...
      {/* ─── Final CTA ─── */}
      <section className="relative mx-auto w-full max-w-[1120px] px-5 pb-24 md:px-8">
          ...
          <div className="relative grid items-center gap-6 md:grid-cols-[1fr_auto]">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.24em] text-white/45">
                {t("pricingExtras.ctaTag", "Empieza hoy")}
              </p>
              <h2 className="mt-3 text-[clamp(26px,3.6vw,40px)] font-black leading-[0.95] tracking-[-0.04em]">
                {t("pricingExtras.ctaTitle", "Crea tu primer smart link gratis")}
              </h2>
            ...
            <a
              href={localizePath("/login", i18n.language)}
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-black tracking-[-0.01em] text-black transition hover:-translate-y-0.5"
            >
              {t("pricingExtras.ctaButton", "Crear cuenta gratis")} <ArrowRight size={15} />
            </a>
          </div>
        </div>
      </section>
...
export const faqs = [
  {
    question: "¿Qué hace exactamente Link My App?",
    answer:
      "Link My App crea un único enlace para tu app. Cuando alguien lo abre, detecta si entra desde iPhone, Android u otro dispositivo y lo lleva automáticamente al destino correcto.",
  },
  {
    question: "¿Puedo crear varios enlaces para una misma app?",
    answer:
      "Sí. La idea es que puedas crear diferentes enlaces para distintas campañas, ubicaciones o canales y saber desde dónde llegan los clics.",
  },
  {
    question: "¿También genera un código QR?",
    answer:
      "Sí. Además del link de descarga, se crea un QR para usarlo en carteles, packaging, mostradores, eventos o campañas físicas.",
  },
  {
    question: "¿Qué pasa si el usuario entra desde ordenador?",
    answer:
      "En ese caso se usa el enlace alternativo que hayas definido, por ejemplo una landing de descarga, una página de información o tu web.",
  },
  {
    question: "¿Necesito tener conocimientos técnicos?",
    answer:
      "No. Solo tienes que pegar el enlace de App Store, el de Google Play, escribir la URL que quieras y crear el link de descarga.",
  },
];

export function FAQSection({ openFaq, setOpenFaq }) {
  const { t } = useTranslation();
  const translatedFaqs = t("landingFaqs.items", { returnObjects: true });
  const items = Array.isArray(translatedFaqs) ? translatedFaqs : faqs;

  return (
    <section id="faqs" className="relative mx-auto w-full max-w-[1240px] px-5 py-20 md:px-8 md:py-24">
      ...
      <div className="mx-auto max-w-3xl text-center">
        <h2 className="text-4xl font-black tracking-[-0.055em] text-black md:text-6xl md:whitespace-nowrap">
          {t("landingFaqs.title")}
        </h2>
        <p className="mx-auto mt-5 max-w-2xl text-base font-medium leading-8 text-black/55 md:text-lg">
          {t("landingFaqs.subtitle")}
        </p>
      </div>
...
function FooterLink({ icon: Icon, label, href, ...props }) {
  return (
    <a
      href={href}
      {...props}
